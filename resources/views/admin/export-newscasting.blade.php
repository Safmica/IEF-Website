...
<head>
    ...
    <title>Export Newscasting</title>
...
<body class="sb-nav-fixed">
                    ...
                    <div class="nav dortmund">
                        ...
                        <div class="collapse" id="collapseExportData" aria-labelledby="headingOne" data-bs-parent="#sidenavAccordion">
                            <nav class="sb-sidenav-menu-nested nav">
                                ...
                                <a class="nav-link active" href="/admin/export-newscasting">Newscasting</a>
                            </nav>
                        </div>
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4 text-dark freshman">Export Data Newscasting</h1>
                    <div class="card mb-4 text-dark mt-3">
                        ...
                        <div class="card-body dortmund">
                            ...
                            <table class="table table-bordered" id="export-newscasting" width="100%" cellspacing="0" class="text-dark">
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>Full Name</th>
                                ...
                                <tbody>
                                    @foreach($newscasting as $index => $newscasting)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $newscasting->full_name }}</td>
                                        <td>Newscasting</td>
                                        <td>{{ $newscasting->gender }}</td>
                                        <td>{{ $newscasting->student_id_number }}</td>
                                        <td>{{ $newscasting->major }}</td>
                                        <td>{{ $newscasting->faculty }}</td>
                                        <td>{{ $newscasting->university }}</td>
                                        <td>{{ $newscasting->nationality }}</td>
                                        <td>{{ $newscasting->phone_number }}</td>
                                        <td>{{ $newscasting->email_address }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#export-newscasting').DataTable({
                dom: 'Bfrtip',
                buttons: [{
                        extend: 'copy',
